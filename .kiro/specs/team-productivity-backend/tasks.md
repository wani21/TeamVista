sqq# Implementation Plan

- [x] 1. Set up project structure and Maven configuration

  - Create Maven project with pom.xml including all required dependencies
  - Set up package structure: model, repository, dto, service, controller, config
  - Create main Application boot class with @SpringBootApplication
  - _Requirements: 7.4_
...
- [x] 2. Implement domain models and enums
  ...
  - [x] 2.1 Create enum classes for Role, TaskStatus, and Priority
    ...
    - Write Role enum with MANAGER and EMPLOYEE values
    - Write TaskStatus enum with PENDING, IN_PROGRESS, COMPLETED values
    - Write Priority enum with LOW, MEDIUM, HIGH values
    - _Requirements: 7.1, 7.3_

  - [x] 2.2 Implement User entity with JPA annotations

    - Create User.java with all fields (id, name, email, password, role, createdAt)
    - Add JPA annotations (@Entity, @Id, @GeneratedValue, @Column with unique constraint on email)
    - Implement explicit getters and setters
    - Add @PrePersist for createdAt timestamp
    - _Requirements: 1.1, 7.3_

  - [x] 2.3 Implement Task entity with JPA annotations and relationships

    - Create Task.java with all fields (id, title, description, status, priority, dueDate, completedDate, assignedTo, createdAt)
    - Add JPA annotations including @ManyToOne for assignedTo relationship
    - Implement explicit getters and setters
    - Add @PrePersist for createdAt timestamp
...
- [ ] 3. Create repository interfaces
  ...
  - [ ] 3.2 Implement TaskRepository interface
    ...
    - Add custom query methods: findByAssignedToId, findByStatus, findByAssignedToIdAndStatus
    - Add query method to find completed tasks by due date for analytics
...
- [ ] 7. Implement controller layer with REST endpoints
  ...
  - [ ] 7.3 Create DashboardController
...
- [ ] 10. Configure application properties and database
  ...
  - [ ] 10.2 Create data.sql initialization script
    - Insert 1 MANAGER user with BCrypt hashed password
